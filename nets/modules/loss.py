...
import numpy as np
import cv2
from utils.tools import fast_hist
import math
...
class CategoryOhemCrossEntroy2d(nn.Module):
    def __init__(self, ignore_label=255, ratio=0.1, num_classes=19, use_weight=False):
        super(OhemCrossEntropy2d, self).__init__()
        self.ignore_label = ignore_label
        self.num_classes = num_classes
        self.ratio = float(ratio)
        self.count = [0 for i in range(num_classes)]
        if use_weight:
            weight = torch.FloatTensor([0.8373, 0.918, 0.866, 1.0345, 1.0166, 0.9969, 0.9754, 1.0489, 0.8786, 1.0023, 0.9539, 0.9843, 1.1116, 0.9037, 1.0865, 1.0955, 1.0865, 1.1529, 1.0507])
            self.criterion = torch.nn.CrossEntropyLoss(weight=weight, ignore_index=ignore_label)
        else:
    ...
    def forward(self, predict, target):
        ...
        predictions = predict.data.max(1)[1].squeeze_(1).cpu().numpy()
        ...
        for lp, lt in zip(predictions_all, gts_all):
            hist += fast_hist(lp.flatten(), lt.flatten(), self.num_classes)
        iu = np.diag(hist) / (hist.sum(axis=1) + hist.sum(axis=0) - np.diag(hist))

        index = iu.argsort()
        index = index[:math.ceil(self.ratio * self.num_classes)]
        # --------------------------------
        # index0 = []
        # for i, iu_temp in enumerate(iu):
        #     if iu_temp < 0.1:
        #         index0.append(i)
        # if len(index0) > len(index):
        #     index = index0
        # --------------------------------
        # count
        for i in index:
        ...
        logger_count.info("count: {}".format(self.count))

        for i_class in range(self.num_classes):
            if i_class not in index:
                gts_all[gts_all == i_class] = self.ignore_label
        target = torch.from_numpy(gts_all).cuda()
